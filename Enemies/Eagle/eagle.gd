...
func set_and_flip() -> void:
	...
	# by subtracting the eagle X position from player X position
	# and get the sign of the number (1 or -1)
	var x_dir = sign(_player_ref.global_position.x - global_position.x)
	
	# Flip the sprite if the player is on the other side of us
	if x_dir > 0:
